...
public class DynresWindow extends Window {
    ...
    public final Defer.Future<List<Preview.Spec>> previews = Preview.Spec.fetch();
    ...
    public class Adder extends Widget implements DropTarget {
        private final List<Defer.Future<BufferedImage>> processing = new LinkedList<>();
        ...
        public void tick(double dt) {
            super.tick(dt);
            for (Iterator<Defer.Future<BufferedImage>> i = processing.iterator(); i.hasNext(); ) {
                Defer.Future<BufferedImage> proc = i.next();
                if (proc.done()) {
                    try {
    ...
    public static class Preview extends Widget {
        ...
        public static class Spec {
            ...
            public static Defer.Future<List<Spec>> fetch() {
        ...
        public static class View extends PView implements Sprite.Owner {
            ...
            public View(Coord sz, Spec spec, Indir<Resource> vres) {
                ...
                this.spec = spec;
                this.vres = vres;
//                basic(Camera.class, new PointedCam(Coord3f.o, 200, (float) Math.PI / 6, (float) Math.PI / 4));
            }
            ...
            protected Camera camera() {
                if (cam == null)
                    cam = new PointedCam();//Coord3f.o, 200, (float) Math.PI / 6, (float) Math.PI / 4
                return (cam);
            }

            DirLight light = new DirLight(new Color(96, 96, 160), new Color(255, 255, 208), Color.WHITE, Coord3f.of(1, 1, 1).norm());

            @Override
            protected void setup(final RenderList rls) {
                ...
                rls.add(light, null);
                cam.update(PointedCam.compute(Coord3f.o, 200, (float) Math.PI / 6, (float) Math.PI / 4));
                rls.prepo(rot);
            }
            ...
            /*public static Volume3f getbounds(Rendered r) {
                Volume3f ret = null;
                tree.add(r);
                for (RenderList.Slot s : r.slots()) {
...
//                        Coord3f mid = bnd.p.add(bnd.n).div(2);
                        GLState.Wrapping n = GLState.compose(spec.st/*, Location.xlate(mid.neg())*/).apply(spr);
                        GLState.nullstate.apply(n);
                    } catch (Loading l) {}
//                }
                if (spr != null)
                    spr.tick(dt);
            ...
            @SuppressWarnings("deprecation")
            public Resource getres() {throw (new UnsupportedOperationException());}
    ...
    public static class PreviewWindow extends Window {
        ...
        public PreviewWindow(BufferedImage img, List<Preview.Spec> previews) {
            ...
            pack();
            display.move(Coord.of((sz.x - display.sz.x) / 2, display.c.y));
        }
        ...
        private class Upload implements Defer.CRunnable {
            volatile byte[] data;
            volatile int off;
        ...
        public void upload() {
            if (upload == null) {
                Defer.later(upload = new Upload());
                uploadbtn.hide();
                prog = add(new Progress(sz.x).val(upload::prog).text(upload::text), uploadbtn.c);
                pack();
            }
        }
    }
    ...
    public void uimsg(String nm, Object... args) {
        if (nm == "add") {
            UID id = UID.of(((Number) args[0]).longValue());
